                                                        ...
                                                        <div class="flex justify-between items-start mb-2">
                                                            <div class="flex items-center gap-2">
                                                                <div class="badge badge-neutral text-xs">
                                                                    #{{ $meeting->id }}</div>
                                                                <div class="badge badge-{{ $meeting->status->color() }} badge-xs">
                                                                    {{ $meeting->status->label() }}
                                                                </div>
                                                            </div>
                                                        </div>                                        <!-- Meeting Details -->
                                        <div class="space-y-1 text-xs text-base-content/70 mb-2">
                                            @if ($meeting->room)
                                                <div class="flex items-center gap-2">
                                                    <x-heroicon-s-map-pin class="size-3 flex-shrink-0" />
                                                    <span class="truncate">{{ $meeting->room->name }}</span>
                                                </div>
                                            @else
                                                <div class="flex items-center gap-2">
                                                    <x-heroicon-s-video-camera class="size-3 flex-shrink-0" />
                                                    <span class="truncate">{{ __('Virtual Meeting') }}</span>
                                                </div>
                                            @endif
                                            ...
                                            @if ($meeting->is_one_on_one)
                                                <div class="flex items-center gap-2">
                                                    <x-heroicon-s-chat-bubble-left-right class="size-3 flex-shrink-0" />
                                                    <span>{{ __('1-on-1') }}</span>
                                                </div>
                                            @endif
                                        </div>                                        <!-- Actions -->
                                        <div class="flex gap-2 mt-3">
                                            <a href="{{ route('issuer.meetings.show', $meeting) }}"
                                                class="btn btn-primary btn-xs rounded-full flex-1">
    ...
    @else
        ...
        <div class="card bg-base-100 shadow-lg">
            <div class="card-body text-center py-12">
                ...
                </div>                <h3 class="text-lg font-medium text-base-content mb-2">{{ $empty }}</h3>
                <p class="text-sm text-base-content/70 max-w-md mx-auto">
                    {{ __('No meetings are currently scheduled. Your meetings will appear here once they are created.') }}
                </p>
            </div>
        </div>
    @endif
