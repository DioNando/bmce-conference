        ...
        <div class="flex items-start justify-between">
            <div>
                <h1 class="text-2xl font-bold text-primary">
                    {{ __('Edit Meeting #:id', ['id' => $meeting->id]) }}
                </h1>
            </div>
        </div>
                ...
                <form action="{{ route('admin.meetings.update', $meeting) }}" method="POST">
                    ...
                    <section class="mb-6 border-b border-base-200 pb-6 grid grid-cols-1 md:grid-cols-3 gap-8">
                        <div>
                            ...
                            <p>
                                {{ __('Update the issuer and venue for this meeting.') }}
                            </p>
                        </div>
                                ...
                                <select id="issuer_id" name="issuer_id" class="select select-bordered w-full" required>
                                    ...
                                    @foreach ($issuers as $issuer)
                                        <option value="{{ $issuer->id }}"
                                            {{ old('issuer_id', $meeting->issuer_id) == $issuer->id ? 'selected' : '' }}>
                                            {{ $issuer->name }}
                                ...
                                <select id="room_id" name="room_id" class="select select-bordered w-full">
                                    ...
                                    @foreach ($rooms as $room)
                                        <option value="{{ $room->id }}"
                                            {{ old('room_id', $meeting->room_id) == $room->id ? 'selected' : '' }}>
                                ...
                                <div class="flex items-center space-x-6 mt-2">
                                    <div class="flex items-center">
                                        <input id="is_one_on_one_1" name="is_one_on_one" type="radio"
                                            ...
                                            {{ old('is_one_on_one', $meeting->is_one_on_one) == 1 || old('is_one_on_one', $meeting->is_one_on_one) === true ? 'checked' : '' }}>
                                    ...
                                    <div class="flex items-center">
                                        <input id="is_one_on_one_0" name="is_one_on_one" type="radio"
                                            ...
                                            {{ old('is_one_on_one', $meeting->is_one_on_one) == 0 || old('is_one_on_one', $meeting->is_one_on_one) === false ? 'checked' : '' }}>
                                ...
                                <select id="status" name="status" class="select select-bordered w-full" required>
                                    @foreach (\App\Enums\MeetingStatus::cases() as $meetingStatus)
                                        <option value="{{ $meetingStatus->value }}"
                                            {{ old('status', $meeting->status->value) == $meetingStatus->value ? 'selected' : '' }}>
                    ...
                    <section class="mb-6 border-b border-base-200 pb-6 grid grid-cols-1 md:grid-cols-3 gap-8">
                        <div>
                            ...
                            <p>
                                {{ __('Change the date and time for the meeting if needed.') }}
                            </p>
                        </div>
                        ...
                        <div class="col-span-2">
                            ...
                            <fieldset class="fieldset mb-4">
                                ...
                                <input type="hidden" id="time_slot_id" name="time_slot_id"
                                    value="{{ old('time_slot_id', $meeting->time_slot_id) }}">

                                <div id="time_slot_container">
                                    <p id="no_issuer_message"
                                        class="italic text-base-content/70 {{ old('issuer_id', $meeting->issuer_id) ? 'hidden' : '' }}">
                                    ...
                                    <div id="time_slots_grid"
                                        class="space-y-6 {{ old('issuer_id', $meeting->issuer_id) ? '' : 'hidden' }}">
        ...
        return $timeSlot->date->format('Y-m-d');
                                            ...
                                            <div>
                                                ...
                                                <div class="flex flex-wrap gap-3">
                                                    @foreach ($dateTimes as $timeSlot)
                                                        <div class="time-slot-card btn rounded-full {{ old('time_slot_id', $meeting->time_slot_id) == $timeSlot->id ? 'btn-primary' : '' }}"
                    ...
                    <section class="mb-6 border-b border-base-200 pb-6 grid grid-cols-1 md:grid-cols-3 gap-8">
                        <div>
                            ...
                            <p>
                                {{ __('Update the list of investors invited to this meeting.') }}
                            </p>
                        </div>
                        ...
                        <div class="col-span-2">
                            ...
                            <fieldset class="fieldset mb-4">
                                ...
                                <x-multi-select name="investor_ids" id="investor_ids" :options="$investors"
                                    :selected="old('investor_ids', $meeting->investors->pluck('id')->toArray())" :placeholder="__('-- Select Investors --')" :required="true" :isMultiple="true"
                    ...
                    <section class="mb-6 grid grid-cols-1 md:grid-cols-3 gap-8">
                        <div>
                            ...
                            <p>
                                {{ __('Update notes or additional details about this meeting.') }}
                            </p>
                        </div>
                        ...
                        <div class="col-span-2">
                            ...
                            <fieldset class="fieldset mb-4">
                                ...
                                <textarea id="notes" name="notes" rows="4" class="textarea textarea-bordered w-full">{{ old('notes', $meeting->notes) }}</textarea>
                                @error('notes')
                                    <p class="label text-error text-sm">{{ $message }}</p>
                                @enderror
                            </fieldset>
